...
class Board:
    ...
    def traverse_left(self, start, stop, step, color, left, skipped=None):
        if skipped is None:
            skipped = []
        moves = {}
        last = []
    ...
    def traverse_right(self, start, stop, step, color, right, skipped=None):
        if skipped is None:
            skipped = []
        moves = {}
        last = []
...
while run:
    clock.tick(FPS)
    if game.winner() is not None and Winner:
        if game.winner() == RED:
            print('Red Win')
        else:
            print('Blue Win')
